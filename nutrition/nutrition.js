document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".nutri").forEach(data => {
    data.value = "";
  });
...
if (track) {
  let btn1 = document.querySelector(`#${track}`);
...
async function submit() {
  ...
  let string = "https://api.spoonacular.com/recipes/findByNutrients?";
  const arr = document.querySelectorAll(".nutri");
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].value !== "" && parseFloat(arr[i].value) >= 0) {
      string += `${arr[i].id}=${arr[i].value}&`;
    }
  }
  string += `number=${document.getElementById("number").value}&`;
  ...
  fetch(string)
    ...
    .then(data => {
      ...
      for (let f = 0; f < data.length; f++) {
        ...
        let re = "";
        for (let [key, value] of Object.entries(data[f])) {
            if( key === "imageType") {
                continue; 
            }
            else if(key==="id"){
                let ke="";
                fetch(`https://api.spoonacular.com/recipes/${value}/analyzedInstructions?apiKey=${apiKey}`)
                .then(res => res.json())    
                .then(data=>{
                    for(let i=0;i<data.length;i++){
                        ke+= `<b>STEPS</b> : <br>`;
                        for(let j=0;j<data[i].steps.length;j++){
                            ke += `${j+1}. ${data[i].steps[j].step}<br>`;
                        }
